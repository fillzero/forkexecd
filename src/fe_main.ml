...
let setup sock cmdargs id_to_fd_map syslog_stdout redirect_stderr_to_stdout env post_script =
...
let _ =
  ...
  while true do
    try
      ...
      match cmd with
	| Fe.Setup s ->
	    let result = setup sock s.Fe.cmdargs s.Fe.id_to_fd_map s.Fe.syslog_stdout s.Fe.redirect_stderr_to_stdout s.Fe.env s.Fe.post_script in
	    (match result with
	      | Some response ->
